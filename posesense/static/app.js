...
function renderFrame() {
  ...
  if (imgReady) {
    ...
    for (const tgt of targets) {
      ...
      const attachedDevices = tgt.devices?.length ? tgt.devices : (tgt.device ? [{ ...tgt.device, placement: tgt.placement }] : []);
      const bound = attachedDevices.length > 0;
      ...
      if (attachedDevices.length) {
        attachedDevices.forEach((d, i) => {
          if (d.placement?.anchor) drawDeviceBadge(d, d.placement, rect, i);
        });
      } else {
...
function drawDeviceBadge(device, placement, rect, index = 0) {
  ...
  const boxH = 36;
  let lx = ax + 14, ly = ay - boxH / 2 + index * 42;
...
function renderUI() {
  ...
  const autoAttached = data.targets.reduce((sum, t) => sum + (t.devices?.length || (t.device ? 1 : 0)), 0);
  document.getElementById("boundCount").textContent = Math.max(data.bindings.length, autoAttached);
  const phones = data.unbound_devices.filter(d => d.is_phone).length
    + data.bindings.filter(b => b.is_phone).length
    + data.targets.reduce((sum, t) => sum + (t.devices || []).filter(d => d.is_phone).length, 0);
  ...
  const attached0 = t0?.devices?.length ? t0.devices : (t0?.device ? [{ ...t0.device, placement: t0.placement }] : []);
  if (attached0.length && identityCard) {
    identityCard.innerHTML = attached0.map(d => {
      const p = d.placement;
      return `
        <div class="id-row"><span class="id-key">${d.icon || "📡"} ${d.device_type}</span><span class="id-val">${d.display_name || d.model || "Unknown"}</span></div>
        <div class="id-row"><span class="id-key">On body</span><span class="id-val highlight">${p?.label || d.likely_body_zone || "—"}</span></div>
        <div class="id-row"><span class="id-key">Signal</span><span class="id-val">${d.is_live_signal === false ? "Windows paired" : `${d.rssi} dBm live`}</span></div>
    ...
    identityCard.innerHTML = `<p class="id-empty">Show one person on camera. Phone/headphones auto-attach when Windows or BLE sees them.</p>`;
  }
  ...
  data.targets.forEach(t => {
    ...
    const devices = t.devices?.length ? t.devices : (t.device ? [{ ...t.device, placement: t.placement }] : []);
    const d = devices[0];
    li.className = (t.person_id === selectedPersonId ? "selected " : "") + (devices.length ? "bound " : "") + (devices.some(x => x.is_phone) ? "phone" : "");
    const idLine = devices.length
      ? devices.map(x => `${x.icon || "📡"} ${x.display_name || x.model}`).join(" · ")
      : (t.ble_name || "Identity unlinked");
    const signal = d && d.is_live_signal === false ? "Paired" : (t.rssi ?? "—");
    li.innerHTML = `<strong>Presence ${t.person_id}</strong><span class="rssi">${signal}</span>
      <div class="sub">${idLine}</div>
      <div class="sub">${devices.length ? "Auto-detected from camera + Bluetooth" : "Waiting for phone/headphones"}</div>`;
    li.onclick = () => {
      selectedPersonId = t.person_id;
  ...
  data.unbound_devices.forEach(d => {
    ...
    const kindTag = d.is_phone ? `<span class="tag tag-phone">Phone</span>` : `<span class="tag">${d.device_type}</span>`;
    const sourceTag = d.is_paired && !d.is_live_signal ? `<span class="tag tag-paired">Paired</span>` : "";
    const zone = d.placement_hint?.label || d.likely_body_zone || "Unknown zone";
    const signal = d.is_live_signal ? `${d.rssi} dBm` : "Paired";
    li.innerHTML = `<strong>${d.icon} ${d.display_name || d.name}</strong>${kindTag}${sourceTag}<span class="rssi">${signal}</span>
      <div class="sub">${d.brand || "Unknown brand"} · ${d.model || "—"}</div>
      <div class="sub">Likely: ${zone} · ${Math.round(d.type_confidence * 100)}% ID</div>
      <div class="sub">${d.scan_note || "BLE radio signature"}</div>`;
    li.onclick = () => {
